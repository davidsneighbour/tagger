...
Options:
  --config <path>         Config JSON (default: tagger.config.json if present)
...
async function loadConfig(cfgPath: string): Promise<RequiredTaggerConfig> {
  ...
  const cacheFile =
    typeof raw.cacheFile === 'string' && raw.cacheFile.trim() ? raw.cacheFile.trim() : defaults.cacheFile;
...
export async function runTagger(argv: string[]): Promise<void> {
